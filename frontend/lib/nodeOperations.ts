...
export const getNodeOperations = (options: FosReactOptions, appState: AppState, setAppState: (state: AppState) => void, nodeRoute: FosRoute, ) => {
    ...
    const deleteOption = async () => {
        ...
        const moveFocusDown = async () => {
            ...
            const downNodeRoute = getDownNode(nodeRoute, appData.data)
            if (downNodeRoute){
        ...
        const moveFocusUp = async () => {
            const { focusChar, nodeDescription } = getNodeInfo(nodeRoute, appData)
            console.log(appData.data.trellisData)
            const upNodeRoute = getUpNode(nodeRoute, appData.data)
            if (upNodeRoute){
        ...
        const createBid = async (todoRoute: FosRoute, bidInfo: {}) => {

        }


        const createBidRequest = async (todoRoute: FosRoute, bidRequestInfo: {}) => {

        }

        const approveBid = async (bidRoute: FosRoute) => {

        }

        const approveFulfillment = async (fulfillmentRoute: FosRoute) => {


        }

        const approvePayment = async (paymentRoute: FosRoute) => {


        }

        const createMergeRequest = async (route: FosRoute) => {


        }

        const addPin = async (route: FosRoute) => {

        }

        const removePin = async (route: FosRoute) => {
