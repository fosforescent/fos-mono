...
const ResourceComponent = ({ 
  ...
  const getUserPromptRecursive = (appData: AppState, nodeRoute: FosPath, nodeDescription: string, parentDescriptions: string[]) => {
  ...
  const pattern = /.*(\{[^{}]*\}).*/m
  const parsePattern = (lAppData: AppState, lNodeRoute: FosPath, response: string): CostData => {
...
export const getCostInfo = (appData: AppState, nodeRoute: FosPath): CostInfo => {
...
const checkCostInfo = (appData: AppState, nodeRoute: FosPath): boolean => {
  const { nodeData } = getNodeInfo(nodeRoute, appData)
