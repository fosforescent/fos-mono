...
import { AppState, FosDataContent, FosPathElem, FosReactOptions, FosRoute } from "../types";
...
export const suggestTaskOptions = async (
  ...
  const { getParentInfo, childRoutes, nodeType, nodeContent } = getNodeInfo(nodeRoute, appState)
  ...
  if (nodeType === "option"){
    ...
    const {
      ...
      acc: { newState: AppState, childRoutes: FosRoute[] }, 
      alternative: {
        description: string,
        steps: string[]
      }, i: number) => {
      ...
      const childInfo = getNodeInfo(childRoute, stateWithChild)

      const optionRoute: FosRoute = [...nodeRoute, ["option", childInfo.nodeId]]
      ...
      const {
        ...
        const newRow = { data: newNodeData, content: []}
    ...
    const stateWithBlankOption = updateNodeContent(stateWithSelf, { data: { description: { content: "" }, alias: nodeContent.data.alias }, content: [childElem] }, newRoute)
