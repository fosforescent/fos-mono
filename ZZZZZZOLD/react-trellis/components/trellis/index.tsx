...
import { TrellisInterfaceOptions,  TrellisMeta,  TrellisNodeClass, TrellisNodeInterface, TrellisSerializedData, TrellisTrail } from '../../trellis/types'

import { DefaultTrellisGraph } from './graph'


// eslint-disable-next-line @typescript-eslint/ban-types
export function MainView <T extends TrellisNodeInterface<T>, S>({ 
  options,
  rootNode,
  disabled,
  state,
  ...
  options?: TrellisInterfaceOptions
  disabled?: boolean
  rootNode: TrellisNodeClass<T, S>
  global: S,
  ...
  // console.log('trellis main')

  const theme = options?.theme ? options.theme : window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light"


  const [activeId, setActiveId] = useState< { id: string, node: T, breadcrumb: boolean } | null>(null);

  const [dragOverInfo, setDragOverInfo] = useState<{ id: string, position: 'above' | 'below' | 'on' | 'breadcrumb', node: T } | null>(null);
  ...
  useEffect(() => {
    ...
    if ( (activeId?.id || null) !== state.draggingNode){
      updateState({ ...state, draggingNode: (activeId?.id || null) })
    }

    if ( (dragOverInfo?.id || null) !== state.draggingOverNode){
      updateState({ ...state, draggingOverNode: (dragOverInfo?.id || null) })
    }
  ...
  const getDragOverInfo = (over: Over | null, active: Active | null): {
    ...
    if (over.data.current?.breadcrumb) {
      return { id: over.id, position: 'breadcrumb', node: over.data.current?.node }
    }
  ...
  function handleDragStart(event: DragStartEvent) {
    ...
    if (active && active.data.current) {
      setActiveId({ id: active.id, node: active.data.current.node, breadcrumb: active.data.current?.breadcrumb });
    }
  ...
  const zoomHelper = (node: TrellisNodeClass<T, S>, trail: string[]): TrellisNodeClass<T, S>[] => {
    ...
    if (node.getId() !== trailHead) {
      console.log('node', node.getId(), trailHead, trail)
    ...
    if (trailTail.length === 0) {
      ...
      console.log('nextNode', node.getChildren(), nextNode, trailTail)
  ...
  const trail = state.zoomRoute.length > 0 ? zoomHelper(meta.trellisNode.getRoot(), state.zoomRoute) : [meta.trellisNode.getRoot()]

  const zoomedNode = trail[trail.length - 1] || rootNode


  const BreadcrumbsComponent = rootNode.components.breadcrumbs
  const RootComponent = rootNode.components.root
      ...
      <div className={`w-full trellis-root ${theme}`} >
        <div className={` bg-background/50 text-primary`}>
          <BreadcrumbsComponent 
            node={rootNode.getInterfaceNode()}
            trail={trail}
            dragOverInfo={dragOverInfo}
            dragging={activeId}
            global={props.global}
            meta={zoomedNode.getMeta()}
            state={state}
          ...
          <div className="w-full">
            {<RootComponent 
              node={zoomedNode.getInterfaceNode()}
              dragging={activeId}
              dragOverInfo={dragOverInfo} 
