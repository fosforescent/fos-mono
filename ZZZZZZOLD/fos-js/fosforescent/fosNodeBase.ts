...
export class FosNodeBase implements IFosNode {
  ...
  init(context: FosContextData) {
    const content = context.nodes[this.id]
    if (!content){
      // eslint-disable-next-line no-console
  ...
  delete() {
    const parentElem = this.getAncestors()[0]
    if (!parentElem){
      throw new Error('no parent element')
    }
    const [parentNode, childIndex] = parentElem
  ...
  setPath(selectionPath: SelectionPath) {
    ...
    if (thisNodeType === "workflow"){
      ...
      if(!selectionPath[thisKey]){
        throw new Error('no selection path found')
      }
      selectedChild.setPath(selectionPath[thisKey])
...
export class FosRootNode extends FosNodeBase {
  ...
  constructor(contextData: FosContextData, setContextData: (contextData: FosContextData) => Promise<void> = async () => {}) {
    ...
    const id = contextData.trail && contextData.trail[0] ? contextData.trail[0]?.[1] : "root"
    const type = contextData.trail && contextData.trail[0] ? contextData.trail[0]?.[0] : "root"
...
export const checkDataFormat = (data: FosContextData) => {
  ...
  const rootNode = data.nodes[rootId]
  if (!rootNode){
    throw new Error('no root node')
  }

  const rootNodeHasTask = rootNode.content.some(([type, id]) => {
    // console.log("key", key, type, id)
    return type === 'workflow'
  })

  if (!rootNodeHasTask){
    throw new Error('root node has no task')
  }
