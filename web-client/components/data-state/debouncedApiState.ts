...
import { FosContextData, FosNodesData } from '@fosforescent/react-client'
...
export const handleDataChange = (appData: DataState) => {

  console.log('handleDataChange', appData)

  return (dataState: FosContextData): DataState => {

    if (dataIsEmpty(dataState) && dataIsEmpty(appData.fosData)){
        const newDataState = {
          fosData: initialDataState.fosData,
          synced: true,
          stored: true,
          locked: false,
        ...
        return newDataState  
    } else if (dataIsEmpty(dataState)){

      return appData

    } else if (dataIsEmpty(appData.fosData)){
      ...
      const mergedData = mergeContexts(appData.fosData, dataState)
...
export const useDebouncedApiState = ({
  ...
  apiData: FosContextData,
  setApiData: (data: FosContextData) => Promise<FosContextData>,
  appData: DataState,
  ...
  React.useEffect(() => {
    ...
    if (_.isEqual(apiData, appData.fosData)){
      return
    }
  ...
  const getRelevantNodes = (data: DataState, depth: number) => {
    ...
    const getIdsHelper = (node: string, depth: number, ids: Set<string>) => {
      ...
      if (depth === 0){
        ...
        const children = data.fosData.nodes[node]?.content
    ...
    const rootId = getRootId(data.fosData)
    ...
    const relevantNodes = [...relevantNodeIds].reduce((acc, id) => {
      const result = data.fosData.nodes[id]
      if (!result){
  ...
  const getData = () => {
    ...
    (async () => {   
      const newState = await setApiData({} as AppData).then(handleDataChange(appData)).catch(handleDataError(appData, setFetching))
      setAppData(newState)
    })()
  }

  const syncData = (newDataState: DataState) => {
    ...
    if (_.isEqual(newDataState.fosData, apiData)){
    ...
    (async () => {
      ...
      const postFosData = {
        ...newDataState.fosData,
        nodes: getRelevantNodes(newDataState, rowDepth)
      }
      
      console.log('postFosData  --- SYNC', postFosData);
      setFetching(true)
      const newState = await setApiData(postFosData).then(handleDataChange(appData)).catch(handleDataError(appData, setFetching))
      setFetching(false)
