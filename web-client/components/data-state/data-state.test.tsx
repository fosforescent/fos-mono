...
import { FosContextData, FosNodeContent } from '@fosforescent/react-client';
...
describe('useDataState hook', () => {
  ...
  test.skip('setAppData updates appState data correctly', async () => {
    ...
    const appData = initialAppState.data.fosData
    ...
    const TestComponent = () => {
      ...
      const defaultContext: FosContextData = {
        focus: {
          route: [["root", rootTaskId], ["task", startTaskId]],
          char: 0
        },
        nodes: {
          [rootTaskId]: {
    ...
    await waitFor(() => {
      ...
      //     "nodes": {
      //       "root": {"collapsed": false, "description": "root", "options": [{"content": [["task", "startTask"]], "description": "latest"}], "selectedOption": 0}, 
